...
Nixはdockerツールを使わなくても、OCI仕様に準拠したコンテナイメージを作成できる強力な機能を持っている。Nixpkgsの`pkgs.dockerTools`モジュールには、以下のような重要な関数が用意されている：

1. **buildImage**: 単一レイヤーのDockerイメージを作成する
2. **buildLayeredImage**: 複数レイヤーのDockerイメージを作成し、依存関係ごとに異なるレイヤーを生成する
3. **streamLayeredImage**: イメージをストリームとして直接出力する

今回のサンプルプロジェクトでは、`dockerTools.buildImage`を使用してコンテナイメージを作成している：
...
この設定によって、クロスコンパイルされたRustアプリケーションを含むOCI仕様準拠のコンテナイメージが生成される。

`contents`パラメータには、イメージに含めるファイルやパッケージを指定する。`config`パラメータでは、エントリポイント、環境変数、公開ポートなどのコンテナ実行時の設定を行える。

Nixを使ったOCIイメージ作成の大きな利点は、イメージのレイヤー構造を完全に制御できることと、特定のネイティブライブラリやツールへの依存なしにクロスアーキテクチャ向けのイメージをクリーンな環境で生成できることである。
